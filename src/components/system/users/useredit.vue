<template>
<el-dialog @open="openDialog" @close="closeDialog" ref="dailog" title="信息编辑" v-draggable :visible.sync="showDialog" width="550px" modal :before-close="handleClose">
  <el-form ref="userData" :inline="true" :disabled="!isEdit" label-width="100px" :model="userData" width="400px" label-position="right">
    <input type="hidden" v-model="userData.USERID" />
    <el-row :gutter="5">
      <el-col :span="12">
        <div>
          <el-form-item label="登录名：" prop="LOGINCODE" :rules="[{required:true,message:'登录名不能为空',trigger:'blue'},{min: 6,
            max: 15,message: '长度在6到15个字符',trigger: 'blur' }]">
            <el-input size="small" v-model="userData.LOGINCODE"  v-bind:disabled="isAdd" placeholder="登录名"></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <div>
          <el-form-item label="性别：">
            <el-switch style="display: block" v-model="userData.gender" :active-value=1 :inactive-value=0></el-switch>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <div>
          <el-form-item label="出生日期：">
            <el-date-picker v-model="userData.birthday" align="left" type="date" placeholder="选择日期" :picker-options="datepickerOption">
            </el-date-picker>
          </el-form-item>
        </div>
      </el-col>
    ...
    <el-row :gutter="5">
      <el-col :span="12">
        <div>
          <el-form-item label="email：" prop="email" :rules="[ { type: 'email', message: '请输入正确的邮箱地址', trigger: ['blur', 'change'] }]">
